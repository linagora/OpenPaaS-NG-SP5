utterance_collapse <-
  function(my_df, utterance_vector_processed, utterance_vector_unprocessed, overlap_threshold) {
    ...
    reduced_my_df_proc = data.frame(matrix(unlist(my_df_new_proc), nrow=length(my_df_new_proc), byrow=T))
    reduced_my_df_unproc = data.frame(matrix(unlist(my_df_new_unproc), nrow=length(my_df_new_unproc), byrow=T))
